...
async function run() {
  ...
  const initScript = path.join(skillRoot, 'scripts', 'init-workspace');
  console.log('🛠️  Initializing workspace...');
  execFileSync('node', [initScript, '--workspace', workspaceRoot]);

  // 4. Create Project
  const projectName = 'aigc-test';
  const createProjectScript = path.join(skillRoot, 'scripts', 'create-project');
  ...
  const generateScript = path.join(skillRoot, 'scripts', 'agent-generate');
