...
async function run() {
  ...
  const initScript = path.join(extractionDir, 'scripts', 'init-workspace');
  try {
  ...
  const createProjectScript = path.join(extractionDir, 'scripts', 'create-project');
