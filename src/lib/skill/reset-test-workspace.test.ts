...
import { resetTestWorkspace } from '../../../scripts/reset-test-workspace.mjs';
...
describe('reset test workspace', () => {
  ...
  it('clears workspace and installs bundled skill into .agent/skills', async () => {
    ...
    await writeFile(path.join(packageRoot, 'bundled-skills', 'mangou', 'scripts', 'http-server.mjs'), 'console.log("ok");');
    ...
    await fs.access(path.join(packageRoot, 'workspace', '.agent', 'skills', 'mangou', 'scripts', 'http-server.mjs'));
  });
});
