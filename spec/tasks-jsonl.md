...
- `appendTaskEvent()` 是纯追加写入接口，不负责查重、不拒绝重复 `pending|submitted` 事件。
- 任务幂等由上层命令或稳定 `id` 负责，`tasks.jsonl` 允许同一 `id` 出现多条中间事件。
- 文件锁只保护“单行 append”临界区，不在锁内做整文件读取或快照推导。
- 写入器在锁被占用时应阻塞等待释放，而不是因为固定重试耗尽而失败。
- 若锁文件超过阈值仍未释放，脚本可将其视为 stale lock 并自动清理。
