...
- `workspace_template/` 的唯一真相源已经迁到 `mangou-ai-motion-comics`；本仓不再承载工作区模板，也不再保留任何仓内 `projects/` 目录。
- runtime 产品层回归测试应优先放在 `mangou-ai-motion-comics`；本仓只保留 dashboard/spec/真正共享 core 所需的测试。
